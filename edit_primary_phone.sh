...
#
# Updates the FIRST_NUMBER value in K3_config_settings file
#
# Usage: edit_primary_phone.sh <phone_number>
#
# Arguments:
...
if grep -q '^FIRST_NUMBER=' "$CONFIG_FILE"; then
    # Replace existing FIRST_NUMBER value
    sed -i "s/^FIRST_NUMBER=.*/FIRST_NUMBER=\"$PHONE_NUMBER\"/" "$CONFIG_FILE"
    log "INFO: Updated FIRST_NUMBER to: $PHONE_NUMBER"
else
    # FIRST_NUMBER doesn't exist, append it
    echo "FIRST_NUMBER=\"$PHONE_NUMBER\"" >> "$CONFIG_FILE"
    log "INFO: Added FIRST_NUMBER: $PHONE_NUMBER"
fi

# Verify the change was made
if grep -q "^FIRST_NUMBER=\"$PHONE_NUMBER\"" "$CONFIG_FILE"; then
    log "SUCCESS: FIRST_NUMBER successfully updated to $PHONE_NUMBER"
