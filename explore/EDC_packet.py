...
def get_modem_info(
    ...
    try:
        ...
        if verbose:
            logging.info(
                f"Modem info retrieved - ICCID: {iccid}, IMEI: {imei}, IMSI: {imsi}"
            )
...
def configure_modem_tcp(
    ...
    try:
        ...
        response = sbc_cmd("AT#SGACT?\r", serial_connection, verbose=verbose)
        ...
        # <sendDataMode>
        # srMode=2: Enable all socket ring unsolicited messages (CRITICAL for command
        #  mode) This enables #SRING notifications when data arrives
        response = sbc_cmd(
...
def send_tcp_packet(
    ...
    try:
        ...
        # Using context 1, packet size 1500, max timeout 90s, connect timeout 600s, tx
        # timeout 50s
        response = sbc_cmd(
        ...
        while (time.time() - start_time) < timeout:
            if serial_connection.in_waiting > 0:
                ...
                # Check for #SRING: <socketId>,<dataLen>[,<data>] or SRING: <socketId>,
                # <dataLen>[,<data>]
        ...
        if sring_line:
            ...
            if len(sring_parts) >= 3:
                ...
                data_len = int(sring_parts[1].strip())
                logging.info(
                    f"#SRING indicates {data_len} bytes available, "
                    "retrieving with AT#SRECV..."
                )
        ...
        response = sbc_cmd(f"AT#SH={socket_id}\r", serial_connection, verbose=verbose)
...
def main():
    ...
    try:
        ...
        event_data = (
            ...
            f"BOOT = 03010007|TSPV=25.21.260-P0F.261803|CCI={iccid}|IMSI={imsi}|"
            f"IMEI={imei}|NET=4G|APN=broadband|IMS=1|SS=067|RSRP=098|RSRQ=011|"
            f"TMP1=+020|TMP2=+020|BAT=1305|ZLST=01|STM=0450946E|UTM=02EBA09E|"
            f"RST=0|PIN=1|THW=1.10 END"
        )
